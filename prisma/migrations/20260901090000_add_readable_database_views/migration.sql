-- Human-readable, read-only database views.
--
-- The application keeps UUID columns in its base tables so foreign-key
-- relationships stay reliable.  Use the views below when browsing data in a
-- database client: they replace relation UUIDs with the corresponding names
-- and deliberately do not expose password hashes.
...
SELECT
  ...
  u."email",
  u."role",
  p."name" AS "preferredPositionName",
  u."createdAt",
  u."updatedAt"
FROM "users" u
LEFT JOIN "positions" p ON p."id" = u."preferredPositionId";
...
CREATE OR REPLACE VIEW "readable_chat_sessions" AS
SELECT
  u."name" AS "userName",
...
CREATE OR REPLACE VIEW "readable_chat_messages" AS
SELECT
  u."name" AS "userName",
  s."title" AS "sessionTitle",
  m."role",
  m."content",
  m."sourceType",
  COALESCE(m."sourceArticleTitle", a."title") AS "sourceArticleTitle",
  m."sourceConfidenceScore",
  m."createdAt"
...
CREATE OR REPLACE VIEW "readable_knowledge_base_articles" AS
SELECT
  a."title",
  a."content",
  c."name" AS "categoryName",
...
CREATE OR REPLACE VIEW "readable_lessons" AS
SELECT
  l."title",
  l."content",
  l."order",
  ...
  l."createdAt"
FROM "lessons" l
LEFT JOIN "users" u ON u."id" = l."createdByUserId";

CREATE OR REPLACE VIEW "readable_quizzes" AS
SELECT
  q."title",
  u."name" AS "createdByUserName",
  l."title" AS "lessonTitle",
  a."title" AS "sourceArticleTitle",
...
CREATE OR REPLACE VIEW "readable_questions" AS
SELECT
...
CREATE OR REPLACE VIEW "readable_quiz_attempts" AS
SELECT
  u."name" AS "userName",
  q."title" AS "quizTitle",
  l."title" AS "lessonTitle",
...
CREATE OR REPLACE VIEW "readable_lesson_progress" AS
SELECT
  u."name" AS "userName",
  l."title" AS "lessonTitle",
  progress."completed",
...
CREATE OR REPLACE VIEW "readable_positions" AS
SELECT
  p."name" AS "positionName",
  p."description",
  p."isActive",
...
CREATE OR REPLACE VIEW "readable_position_skills" AS
SELECT
  p."name" AS "positionName",
  ...
  skill."createdAt",
  skill."updatedAt"
FROM "position_skills" skill
JOIN "positions" p ON p."id" = skill."positionId";

CREATE OR REPLACE VIEW "readable_user_skill_scores" AS
SELECT
  u."name" AS "userName",
  p."name" AS "positionName",
  skill."name" AS "skillName",
...
CREATE OR REPLACE VIEW "readable_skill_score_events" AS
SELECT
  u."name" AS "userName",
  p."name" AS "positionName",
  skill."name" AS "skillName",
...
LEFT JOIN LATERAL (
  SELECT string_agg(skill."name" || ': ' || split_part(snapshot_item, ':', 2), ' | ' ORDER BY skill."name")
...
CREATE OR REPLACE VIEW "readable_quiz_question_skills" AS
SELECT
  q."title" AS "quizTitle",
  question."questionText",
  skill."name" AS "skillName",
