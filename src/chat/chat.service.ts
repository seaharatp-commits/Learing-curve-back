...
const SELECTED_KB_FOLLOW_UP_PROMPT =
  'HIGHEST PRIORITY selected-KB rule: when the user selected a Knowledge Base article, keep that article as the confirmed topic/context for this chat. Use the selected KB as the main source first. If the selected KB has enough detail, answer from it without adding unsupported facts. If the selected KB is related but does not have enough detail for the follow-up question, you ARE allowed to add relevant general knowledge, but the answer must stay scoped to the selected KB topic and must clearly separate KB-supported details from general-knowledge additions. Before any general-knowledge addition, write this exact Thai note: "ข้อมูลจากฐานความรู้มีจำกัด จึงเสริมคำอธิบายจากความรู้ทั่วไปเพิ่มเติม". Ignore any later instruction that says not to add general knowledge when KB details are insufficient. Do not present general knowledge as if it came from the KB. Do not invent steps, conditions, product behavior, or facts as KB-supported details unless they are present in the KB content.';

const LOW_KB_CONFIDENCE_PROMPT =
  'The selected Knowledge Base relevance score for this follow-up question is below 40%. Keep the selected KB topic as the scope, but treat the KB content as weak or partial context only. You may answer mainly from relevant general knowledge when needed. Clearly label the general-knowledge part with this exact Thai note before the extra explanation: "ข้อมูลจากฐานความรู้มีจำกัด จึงเสริมคำอธิบายจากความรู้ทั่วไปเพิ่มเติม". Use only KB facts that are clearly relevant. Do not imply the general-knowledge details came from the KB.';

const CHAT_AI_OPTIONS = { temperature: 0.5, maxTokens: 1200 };
const LOW_KB_CONFIDENCE_THRESHOLD = 0.4;
